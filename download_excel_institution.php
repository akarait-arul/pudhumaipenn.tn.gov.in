...
$header = array("SI No","Institution Type","Total No of Institutions","No of Institutions logged in","No of Institutions not logged in","Total Application Received","Application auto-approved for all stages","Total Pending","Only School Pending","Only Bank Pending","Both School and Bank Pending");
...
$objPHPExcel->getActiveSheet()->mergeCells('A1:K1')->setCellValue('A1', 'Pudhumai Penn Scheme - Application Status as on '.date('d-M-Y').' Institution Wise');
...
for($i = 1; $i <= sizeof($data_instituion); $i++)
{
	
	$pending = ((int)$data_instituion[$i]['Total Application Received'] - (int)$data_instituion[$i]['Application auto-approved for all stages']);
	array_push($result_arr, array(
						
		"S No" => (string)$i,
		"Institution Type" => $data_instituion[$i]['institution_type'],
		"Total No of Institutions" => $data_instituion[$i]['Total No of Institutions'],
		"No of Institutions logged in"=> $data_instituion[$i]['No of Institutions logged in'],
		"No of Institutions not logged in"=> $data_instituion[$i]['No of Institutions not logged in'],
		"Total Application Received"=> $data_instituion[$i]['Total Application Received'],
		"Application auto-approved for all stages"=> $data_instituion[$i]['Application auto-approved for all stages'],
		"Total Pending"=>(string)$pending,
		"Only School Pending"=> $data_instituion[$i]['Only School Pending'],
		"Only Bank Pending"=> $data_instituion[$i]['Only Bank Pending'],
		"Both School and Bank Pending"=> $data_instituion[$i]['Both School and Bank Pending']		
	));
	
	$total_no_institution += (int)$data_instituion[$i]['Total No of Institutions'];
	$no_of_institutions_logged_in += (int)$data_instituion[$i]['No of Institutions logged in'];
	$no_of_institutions__not_logged_in += (int)$data_instituion[$i]['No of Institutions not logged in'];
	$total_application_received += (int)$data_instituion[$i]['Total Application Received'];
	$application_auto_approved_all_stages += (int)$data_instituion[$i]['Application auto-approved for all stages'];
	$total_pending += $pending;
	$only_school_pending += (int)$data_instituion[$i]['Only School Pending'];
	$only_bank_pending += (int)$data_instituion[$i]['Only Bank Pending'];
	$both_school_bank_pending += (int)$data_instituion[$i]['Both School and Bank Pending'];
...
$objPHPExcel->getActiveSheet()->getStyle('A1:K'.$total_count_row.'')->applyFromArray($styleArray);
